...
for( $counter = 1; $counter <= $numdaysinmonth; $counter ++ )
{
    ...
    else
    {
        foreach($result AS $row)
        {
            ...
            $out.= '<div class="calitem'.$row->date_id.'" style="background-color:'.$row->bgcolor.'" >'.$row->title.' </div> <br/><script type="text/javascript">$(".calitem'.$row->date_id.'").wTooltip({content: "'.$popup.'"});</script>';
        }
    }    
    $out.="</td>\n";
