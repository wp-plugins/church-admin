 ...
 */    
    ...
    {//results
        ...
        foreach($results AS $row)
        {
            ...
            $delete='<a href="'.wp_nonce_url('admin.php?page=church_admin/index.php&amp;action=delete_sermon_series&amp;id='.intval($row->series_id),'delete_sermon_series').'">Delete</a>';
 ...
 */
    ...
    {//non empty $id
        ...
        {//end sure so delete
            ...
            ca_podcast_xml();//update podcast feed
            echo'<div class="updated fade">'.esc_html($data->file_title).' '.__('from','church-admin').' '.esc_html($data->series_name).' '.__('deleted','church-admin').'</p></div>';
            ca_podcast_list_files();
        }//end sure so delete
        else
        {
    ...
    else{echo'<p>No file specified '.intval($id).'</p>';}
}
...
function ca_podcast_file_delete($file_name=NULL)
{
    ...
    if($file_name &&is_file($path.basename($file_name)))
    {
        ...
        echo'<div class="updated fade"><p>'.esc_html(basename($file_name)).' deleted</p></div>';
...
function ca_podcast_xml()
{
    ...
    if(!empty($results)&&!empty($settings['title']))
    {
        ...
        foreach($results AS $row)
        {
            ...
            if(!empty($row->file_name)){$output .= '<enclosure url="'.ent2ncr($url.$row->file_name).'" length="'.filesize($path.$row->file_name).'" type="audio/mpeg" />';$output .= '<guid>'.ent2ncr($url.$row->file_name).'</guid>';}else{$output .= '<enclosure url="'.ent2ncr($row->external_file).'" length="" type="audio/mpeg" />';$output .= '<guid>'.ent2ncr($row->external_file).'</guid>';}
