...
function church_admin_whosin($id)
{
	...
	$out='';
	$group=$wpdb->get_row('SELECT * FROM  '.CA_SMG_TBL.' WHERE id="'.esc_sql($id).'"');
	if(!empty($group))
	{
		//group details
		$out.=sprintf( '<h2>%1$s %2$s %3$s</h2>', __( 'Who is in', 'church_admin' ),$group->group_name,__('group','church-admin') );
		$out.='';
		$out.='<table class="form-table"><tbody>';
		$out.='<tr><th scope="row">'.__('Leader(s)','church-admin').':</th><td>'.church_admin_get_people($group->leader).'</td><td rowspan=3><img class="alignleft" src="http://maps.google.com/maps/api/staticmap?center='.$group->lat.','.$group->lng.'&zoom=13&markers='.$group->lat.','.$group->lng.'&size=200x200&sensor=FALSE"/></td></tr>';
		...
		if(!empty($peopleresults))
		{
				...
				foreach($peopleresults AS $row)
				{
					...
					{	
						$out.='<tr><td>'.esc_html($row->name).'</td><td>'.$attendance[$row->smallgroup_attendance].'</td><td><a href="mailto:'.$row->email.'">'.esc_html($row->email).'</a></td><td><a href="call:'.$row->email.'">'.esc_html($row->mobile).'</td></tr>';
					}
				}
				$out.='</tbody></table>';
		}
