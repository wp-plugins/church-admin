...
if(!empty($result))
{
  foreach($result AS $row)
	...
	$out.='<p '.$class.'><span itemprop="summary">'.strtoupper(esc_html($row->title)).'</span><br/>';
	$out.='<time itemprop="startDate" datetime="'.date('c',strtotime($row->start_date.' '.$row->start_time)) .'">'.esc_html($date).' '.strtolower($row->start_time).' </time><br/>';
	$out.='	<span itemprop="location" itemscope itemtype="http://data-vocabulary.org/​Organization"><span itemprop="name">'.esc_html($row->location).'</span></span></p></div>';
    }
