 ...
 */ 
	...
	if(isset($_GET['action']) && ($_GET['action']=='small_groups'||$_GET['action']=='edit_small_group'||$_GET['action']=='delete_small_group'))
	{
		wp_enqueue_script('google_map_script', 'http://maps.googleapis.com/maps/api/js?sensor=false','',NULL);
	...
	if(!empty($_POST['church_admin_search'])||(isset($_GET['action']) && ($_GET['action']=='church_admin_rota_list'||$_GET['action']=='rota'||$_GET['action']=='church_admin_edit_rota')))
    {
    ...
    if(isset($_GET['action'])&& ($_GET['action']=='church_admin_edit_people'||$_GET['action']=='view_class'||$_GET['action']=='church_admin_add_calendar'||$_GET['action']=='church_admin_series_event_edit'||$_GET['action']=='church_admin_single_event_edit'||$_GET['action']=='church_admin_edit_attendance'||$_GET['action']=='church_admin_new_edit_calendar'||$_GET['action']=='edit_kidswork'||$_GET['action']=='individual_attendance'||$_GET['action']=='edit_class'))
    {
    ...
    if(isset($_GET['action'])&& ($_GET['action']=='edit_hope_team'||$_GET['action']=='view_class'||$_GET['action']=='permissions'||$_GET['action']=='edit_file'||$_GET['action']=='file_add'||$_GET['action']=='church_admin_edit_rota'))
...
function church_admin_main() 
{
	...
	//sermon podcasts
       ...
       case 'church_admin_view_department':check_admin_referer('view_department');require_once(plugin_dir_path(__FILE__).'includes/departments.php');church_admin_view_department($department_id);break;
	    ...
	    //default
	   default:church_admin_people_main();break;
	   
	}
    }else church_admin_people_main();
...
function church_admin_backup()
{
    ...
    if(defined(OLD_CHURCH_ADMIN_VERSION))$content.='UPDATE '.$wpdb->prefix.'options SET option_value="'.OLD_CHURCH_ADMIN_VERSION.'" WHERE option_mame="church_admin_version";'."\r\n";
	$length = 10;
